...
func DecodeVehicle(vin string) (*vinv1.Vehicle, bool) {
	...
	if isNA {
		...
		// "A / B: These characters often delineate the generation and chassis height of the FH/FM heavy-duty platforms."
		// "H / J: Historically associated with medium-duty ranges like the FL or FE."
		// This is ambiguous. "A... denotes a standard FH/FM".
		// We need more specific mapping if possible.
		// However, the doc says "The Chassis Number ... supersedes the full VIN".
		// But we only have VIN.
		// Let's look at the "Reference Tables" or "Deep Decoding" section.
		// It says "A / B ... FH/FM". It doesn't distinguish FH from FM clearly just by A/B.
		// Wait, look at: "YV2A... frequently denotes a standard FH/FM".
		// Is there ANY way to distinguish FH vs FM?
		// The doc says "Position 9 ... encodes Gearbox Type".
		// The doc *does not* provide a clear table for FH vs FM vs FMX in the VDS section for Global.
		// It says "decoding logic ... is more internal".
		// But let's look at `proto/wayplatform/connect/vin/v1/model.proto` again. We have `FH`, `FM`, `FMX`.
		// If we can't distinguish, maybe we leave Model UNSPECIFIED but Brand VOLVO_TRUCKS?
		// Or do we make a best guess?
		// "A / B" -> FH/FM.
		// "H / J" -> FL/FE.
		// Without better granularity, we might have to stop at Brand or return unspecified model.
		// However, let's check if there are specific known patterns.
		// Common knowledge (outside the doc, but trying to stick to doc):
		// The doc is very thorough on NA, less on Global VDS specific model mapping.
		// It says: "In the YV2 format, Positions 4–9 ... map to Volvo's internal 'Variant' codes."
		// It seems we can't reliably determine FH vs FM from just the VDS based *only* on this text.
		// But we can definitely do NA models.
		// Let's implement what we can.
		// Actually, I can try to map the generic families if the code allows.
		// But since I can't be sure, I will leave Global models unspecified unless I find a clue.
		// Wait, look at Table 1 (NA) - very clear.
		// Table 6 just compares structures.
		// I will stick to NA model decoding for now as it's deterministic.
		// For Global, I will just identify it as Volvo Trucks.
	}
